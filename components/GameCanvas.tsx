...
const NiaCharacter: React.FC<{ feedback: boolean | null }> = ({ feedback }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className={`text-8xl ${animationClass}`}>👧</div>
      <p className="text-2xl font-bold text-white mt-2">Nia</p>
    </div>
  );
};
...
const AlienCharacter: React.FC<{ feedback: boolean | null }> = ({ feedback }) => {
  ...
  return (
    <div className="flex flex-col items-center">
       <div className={`text-8xl ${animationClass}`}>👽</div>
       <p className="text-2xl font-bold text-white mt-2">Alien</p>
    </div>
  );
};
...
const VisualScene2D: React.FC<{ problem: Problem }> = ({ problem }) => {
    ...
    const renderGroup = (count: number, keyPrefix: string) => (
      <div className="flex flex-wrap justify-center items-center gap-2 p-2 bg-white/20 rounded-lg">
        {Array.from({ length: count }).map((_, i) => (
          <div key={`${keyPrefix}-${i}`} className="text-4xl">
            {Math.random() > 0.5 ? '🪐' : '⭐'}
          </div>
        ))}
      </div>
    ...
    return (
        <div className="flex justify-center items-center gap-4 my-8 min-h-[200px]">
            {isAddition ? (
                <>
                ...
                <div className="flex flex-wrap justify-center items-center gap-4">
                  {Array.from({ length: groups }).map((_, i) => (
                      <div key={`group-${i}`} className="flex flex-col items-center gap-2 p-2 bg-white/20 rounded-lg">
                          {Array.from({ length: itemsPerGroup }).map((_, j) => (
                              <div key={`item-${i}-${j}`} className="text-3xl">
                                  {Math.random() > 0.5 ? '🪐' : '⭐'}
                              </div>
                          ))}
                      </div>
...
const GameCanvas: React.FC<GameCanvasProps> = ({ problem, feedback, activeDialogue, isNiaResponding }) => {
  return (
    <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-indigo-800 via-purple-800 to-slate-900 overflow-hidden flex flex-col justify-between items-center p-4">
      ...
      <div className="w-full flex justify-around items-end px-8 pb-40">
